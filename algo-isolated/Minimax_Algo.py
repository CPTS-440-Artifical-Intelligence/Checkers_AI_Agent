...
class CheckersMinimax:
    ...
    @staticmethod
    def __max_value(depth_limit: int, current_depth: int, is_maximizing_red_player: bool, mapped_board_state: MappedCheckerBoard, alpha: float, beta: float) -> tuple[float, MappedCheckerBoard, float]:
        ...
        if CheckersMinimax.__is_cutoff(mapped_board_state, depth_limit, current_depth):
            return CheckersMinimax.__eval(mapped_board_state, is_maximizing_red_player), None;
        ...
        for child in MappedCheckerBoard.generateAllPossibleNextMappedBoardStates(mapped_board_state, is_maximizing_red_player):
            ...
            value_2, move_2, beta = CheckersMinimax.__min_value(depth_limit, current_depth + 1, is_maximizing_red_player, child, alpha, beta);
            ...
            if value >= beta: 
                return value, move, alpha;
        
        # return the greatest value and move for the maximizing player and also return alpha for pruning
        return value, move, alpha;
    
    @staticmethod
    def __min_value(depth_limit: int, current_depth: int, is_maximizing_red_player: bool, mapped_board_state: MappedCheckerBoard, alpha: float, beta: float) -> tuple[float, MappedCheckerBoard, float]:
        # stop the search once a win occurs or depth limit is reached
        if CheckersMinimax.__is_cutoff(mapped_board_state, depth_limit, current_depth):
            return CheckersMinimax.__eval(mapped_board_state, is_maximizing_red_player), None;
        ...
        for child in MappedCheckerBoard.generateAllPossibleNextMappedBoardStates(mapped_board_state, not is_maximizing_red_player):
            ...
            value_2, move_2, alpha = CheckersMinimax.__min_value(depth_limit, current_depth + 1, is_maximizing_red_player, child, alpha, beta);
    ...
    @staticmethod
    def heuristic_alpha_beta_minimax(depth_limit: int, checker_board_state: any, is_maximizing_red_player: bool) -> MappedCheckerBoard:
        value, move, alpha = CheckersMinimax.__max_value(depth_limit, 0, is_maximizing_red_player, MappedCheckerBoard.generateMappingFromCheckerBoard(checker_board_state), -math.inf, math.inf);
        return move;
